...
Comparison between two .ct files of a similar sequence, but vary in analysis parameters (e.g. inclusion of probing constrants or varying scanning parameters)
Command line to run the code: python ct_compare.py ref_file.ct target_file.ct output_name
"""
import sys
import os

r_bpij = []				#A list which will hold the i and j values for the reference .ct file, the 'r' prefix is used throughout on variables relating to the reference file
t_bpij = []				#A list which will hold the i and j values for the target .ct file, the 't' prefix is used throughout on variables relating to the target file

filename1 = sys.argv[1]				#The first system argument, this is the reference .ct file
filename2 = sys.argv[2]			    #The second system argument, this is the target .ct file for comparison to the reference
...
with open(f"{outputname}.txt", "w") as file:      #Creates and opens an outputfile with specified output name
    ...
    file.write(third_header + '\n' + f"Number of consistent nts/pairings: {consistent_pairings}" + '\n' + f"Number of conflicting nts/pairings: {conflicting_pairings}" + '\n' + f"Percent consistency between files: {pct_consistent}%\n\n")
